...
const speedOptions = [
  ...
  { name: 'medium', min: 20, max: 40, weight: 0.6 },
...
function getWeightedRandomSpeed() {
  ...
  const duration =
    Math.random() * (selectedSpeedCategory.max - selectedSpeedCategory.min) +
    selectedSpeedCategory.min;
...
function createSingleBubble(container, backgroundType = 'blue', startHidden = false) {
  ...
  const driftPatterns = ['lazyFloatWithDrift', 'lazyFloatWithDrift2', 'lazyFloatWithDrift3'];
  ...
  bubble.classList.add('bubble', 'bubble-3d', `bubble-${bubbleSize}`, randomColor);
  ...
  const exponentialDelay =
    Math.pow(Math.random(), BUBBLE_ANIMATION_CONFIG.distribution) *
    BUBBLE_ANIMATION_CONFIG.maxDelay;
...
function initializeTypewriter() {
  try {
    ...
    const heroText =
      'Tired of high chemical costs, inconsistent water quality, and constant maintenance? HydroCav offers the solution!';
